...
import { CheckCircle, Package, Home, MapPin, Loader2 } from 'lucide-react';
...
export default function CheckoutSuccess() {
  ...
  useEffect(() => {
    ...
    const loadOrder = async () => {
      try {
        const orderData = await getOrder(externalReference);
        setOrder(orderData);
      } catch (error: any) {
  ...
  // Limpar localStorage quando sucesso
  useEffect(() => {
            ...
            <>
              ...
              <div className="mb-6 p-4 bg-green-50 rounded-lg border border-green-200">
                <p className="text-sm text-green-800">
                  <strong>Próximos passos:</strong> Produção será iniciada após confirmação do pagamento.
                </p>
              </div>
            </>
          )}
